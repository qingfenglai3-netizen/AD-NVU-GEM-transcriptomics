...
(R/'intermediate/SEAAD_APOE_frozen_spec.json').write_text(json.dumps(spec,indent=2))
meta=pd.read_excel(R/'intermediate/SEAAD_official_donor_metadata.xlsx').rename(columns={'Donor ID':'donor_id','Age at Death':'age','Sex':'metadata_sex','APOE Genotype':'genotype','Cognitive Status':'cognitive_status'})
...
sea=pd.read_csv(R/'results/SEAAD_GEM_donor_values.csv');d=sea.merge(meta,on='donor_id',how='left',validate='m:1',indicator=True)
