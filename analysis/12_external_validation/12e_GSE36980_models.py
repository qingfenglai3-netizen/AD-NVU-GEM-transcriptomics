...
R=Path(r'<WORKDIR>')
src=Path(r'<WORKDIR>\AD-NVU-GEM-transcriptomics\analysis\11_translational\11a_bulk_multimodel_cross_validation.py')
...
tr=pd.read_csv(R/'results/discovery_GSE132903_labelblind_mean_features.csv');te=pd.read_csv(R/'results/external_GSE36980_features.csv')
...
pd.DataFrame(rows).to_csv(R/'results/external_model_performance.csv',index=False)
pd.DataFrame(predrows).to_csv(R/'results/external_model_predictions.csv',index=False)
