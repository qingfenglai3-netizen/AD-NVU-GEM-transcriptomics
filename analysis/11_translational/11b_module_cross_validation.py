...
d=pd.read_csv(R/'results/discovery_GSE132903_labelblind_mean_features.csv');X=d.iloc[:,2:].to_numpy(float);y=d.group.eq('AD').to_numpy(int)
...
pd.DataFrame(rows).to_csv(R/'results/ML_labelblind_OOF_summary.csv',index=False)
pd.DataFrame(prs).to_csv(R/'results/ML_labelblind_OOF_predictions.csv',index=False)
(R/'intermediate/ML_labelblind_warnings.json').write_text(json.dumps(ws,indent=2))
