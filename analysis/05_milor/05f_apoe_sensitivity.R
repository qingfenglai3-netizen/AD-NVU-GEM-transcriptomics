...
set.seed(20260910)
out<-'<WORKDIR>/results'
dm<-read.csv(file.path(out,'donor_APOE_manifest.csv'));nc<-read.csv(file.path(out,'vascular_pseudobulk_ncells.csv'))
...
pb<-as.matrix(read.csv(file.path(out,'vascular_pseudobulk_counts.csv'),row.names=1,check.names=FALSE));genes<-read.csv(file.path(out,'vascular_DE_cohort_adjusted.csv'))$gene
