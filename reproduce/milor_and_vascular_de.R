...
set.seed(20260910)
base<-normalizePath(Sys.getenv('GEM_REPO',getwd()),winslash='/'); input<-file.path(base,'data')
out<-Sys.getenv('GEM_OUTPUT',file.path(base,'results'));dir.create(out,recursive=TRUE,showWarnings=FALSE)
dm<-read.csv(file.path(input,'donor_APOE_manifest.csv'))
summary<-list()
for(n in c('Cerebrovascular','Astrocytes','Microglia')) {
 counts<-as.matrix(read.csv(file.path(input,paste0(n,'_nhood_counts.csv')),row.names=1,check.names=FALSE));x<-list(da=read.csv(file.path(input,paste0(n,'_original_nhood_results.csv'))))
 sd<-dm[match(colnames(counts),dm$SampleID),];rownames(sd)<-sd$SampleID
 ...
 for(model in c('diagnosis_only','cohort_adjusted','D1_APOE_sex_adjusted')){
  ...
  y<-DGEList(as.matrix(counts)[,rownames(dd),drop=FALSE]);y<-calcNormFactors(y,method='TMM')
...
pb<-as.matrix(read.csv(file.path(input,'vascular_pseudobulk_counts.csv'),row.names=1,check.names=FALSE))
nc<-read.csv(file.path(input,'vascular_pseudobulk_ncells.csv'));keep<-nc$sample_id[nc$n_cells>=20]
